...
fn generate_vertex_buffer(@builtin(global_invocation_id) gid: vec3u)
{
    ...
    vertex_buffer[e    ] = mix(xy_min, xy_max, f32(gid.x)/(GRID_SIZE-1));
    vertex_buffer[e + 1] = mix(xy_max, xy_min, f32(gid.y)/(GRID_SIZE-1));
    vertex_buffer[e + 2] = 0.0;
    vertex_buffer[e + 3] = f32(gid.x)/(GRID_SIZE-1);
    vertex_buffer[e + 4] = f32(gid.y)/(GRID_SIZE-1);
