...
{
    if gid.x >= grid.resolution.x || gid.y >= grid.resolution.y {
        return;
    }
    ...
    vertex_buffer[e    ] = mix(grid.x_range.x, grid.x_range.y, f32(gid.x)/f32(grid.resolution.x-1));
    vertex_buffer[e + 1] = mix(grid.y_range.y, grid.y_range.x, f32(gid.y)/f32(grid.resolution.y-1));
